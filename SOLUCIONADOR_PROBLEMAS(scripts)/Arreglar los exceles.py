...
for acti in ACTIVOS:
    ACTIVO=pd.read_csv(r"C:\Users\Personal\Downloads\pythonProjects_MT5\LeerDatosFromMT5\\"+acti+"Marzo.csv").set_index("time") #MODIFICAR >>>>>>>>>>>>>
    ACTIVO.index=pd.DatetimeIndex(ACTIVO.index).strftime( "%Y-%m-%d %H:%M:%S")
    ACTIVO = ACTIVO[~ACTIVO.index.duplicated(keep='first')]
    ACTIVO=ACTIVO['2022-03-01':'2022-03-31'] # MODIFICAR  >>>>>>>>>>>>>>>>>>>>>>>>>>> cambiar el mes!!!
    ACTIVO.to_csv(r"C:\Users\Personal\Downloads\pythonProjects_MT5\LeerDatosFromMT5\\"+acti+"Marzo.csv")   ##MODIFICAR >>>>>>>>>>>>>
